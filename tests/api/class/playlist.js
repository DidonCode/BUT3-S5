class Playlist {
	constructor(id, owner, title, description, image, isPublic) {
		(this.id = id.toString()), (this.owner = owner), (this.title = title), (this.description = description), (this.image = image), (this.isPublic = isPublic);
	}

	isValid() {
		if (!this.id || typeof this.id !== 'string') return false;
		if (!this.owner || typeof this.owner !== 'number') return false;
		...
		if ((this.isPublic != 0 && this.isPublic != 1) || typeof parseInt(this.isPublic) !== 'number') return false;
