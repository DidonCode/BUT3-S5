...
function updateList(){
    ...
    for(let i = 0; i < playerList.children.length; i++){
        i == sessionStorage.getItem("player-index") ? 
        playerList.children[i].classList.add("selected-card") : 
        playerList.children[i].classList.remove("selected-card");
    }
}
...
function loadButifyPlayer(video){
    ...
    previous.onclick = function() { previousSound(); }
    next.onclick = function() { nextSound(); }
...
function onPlayerReady(event) {
    ...
    play.onclick = function() { eventPlayer.playVideo(); }
    pause.onclick = function() { eventPlayer.pauseVideo(); }
